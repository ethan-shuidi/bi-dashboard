...
- `CPO = 广告花费 / 广告订单量`，按当前展示币种显示；系列和周期汇总按总广告花费除以总广告订单量计算，订单量为零或缺失时显示“—”。
...
- 广告策略看板独立按周、站点、店铺和系列筛选；店铺不选时查询该站点下全部店铺。表格底部显示当前筛选范围的汇总行，点击量、广告花费、广告销售额、广告订单量、广告销量求和，CPC、ACOS、ROAS 和广告 CVR 按汇总后的分子、分母重新计算。
...
- 看板浏览器保存动作通过 `GET /api/dashboard/write-token` 获取 30 分钟短期写权限；该权限绑定编辑者、允许来源和过期时间，不能替代或暴露 `SYNC_API_KEY`。
