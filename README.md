...
- 销量、净销售额、订单量、B2B、Session-Total、CVR、点击、广告花费、广告销量、广告订单量、CTR、CPC、广告CVR、ACOS 统一取领星产品表现接口。
- `ACoAS` 根据产品表现接口返回的广告花费和净销售额计算；`广告销量占比` 保留原方式计算。
- `ACoAS = 广告花费 / 净销售额`。
- `广告销量占比 = 广告归因销量 / 总销量`，不能使用 `adv_rate` 代替。
- 看板“净销售额”明确读取产品表现接口的 `net_amount`（净销售额）字段，不读取 `amount` / `totalSalesAmount`（销售额）字段。
- 系列和周期汇总行对产品表现明细做系统汇总；比例类指标按产品表现明细的对应分母加权汇总。
