...
function setCampaignDraft(campaign, field, value) { const draft = campaignDrafts.value[campaignKey(campaign)]; if (!draft) return; draft[field] = value || ""; if (field === "strategy" && draft.strategy === "/") draft.series = "" }
...
async function saveAllNotes() {
  ...
  saving.value = "notes"; error.value = ""
  try {
  ...
  } catch (e) { error.value = e.message || "优化方向保存失败" } finally { saving.value = "" }
}
async function saveAllCampaigns() {
  ...
  const invalid = items.find((item) => classificationError(item.strategy, item.series))
  if (invalid) {
    error.value = `广告活动“${invalid.campaign_name || invalid.campaign_id}”：${classificationError(invalid.strategy, invalid.series)}`
    showToast("策略和系列必须同时设置或同时留空")
    return
  }
  ...
  try {
    ...
    await load()
  } catch (e) { error.value = e.message || "广告活动归类保存失败" } finally { saving.value = "" }
}
...
<template>
  <el-config-provider :locale="zhCn"><section class="strategy-board-panel">
    <div class="strategy-board-head"><div><span class="section-label">广告后台数据</span><h2>广告策略看板</h2><p>按周、站点、店铺和系列筛选；展开后可给每条广告活动设置策略和系列。</p></div><div class="strategy-board-actions"><button ref="configButton" class="strategy-config-button" type="button" @click="toggleConfig">列配置</button><button class="strategy-refresh-button" type="button" :disabled="loading" @click="load">刷新</button><button class="strategy-note-save strategy-save-all-notes" type="button" :disabled="loading || saving === 'notes' || !groups.length" @click="saveAllNotes">{{ saving === "notes" ? "保存中" : "保存所有优化方向" }}</button></div></div>
