...
function startNameResize(event) { event.preventDefault(); event.stopPropagation(); resizeCleanup?.(); const startX = event.clientX; const startWidth = nameColumnWidth.value; const move = (moveEvent) => { nameColumnWidth.value = Math.max(260, Math.min(720, Math.round(startWidth + moveEvent.clientX - startX))) }; const stop = () => { window.removeEventListener("pointermove", move); window.removeEventListener("pointerup", stop); resizeCleanup = null }; resizeCleanup = stop; window.addEventListener("pointermove", move); window.addEventListener("pointerup", stop, { once: true }) }
