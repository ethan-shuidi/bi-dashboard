...
const allCategoryFilter = "全部分类"
const categoryFilterOptions = [allCategoryFilter, ...categoryOptions]
...
const selectedCategories = ref([allCategoryFilter])
...
const categoryFilterEnabled = computed(() => !selectedCategories.value.includes(allCategoryFilter))
...
function handleCategoryFilterChange(value) {
  ...
  if (!next.length) {
    selectedCategories.value = [allCategoryFilter]
    return
  }
  if (next.includes(allCategoryFilter)) {
    const wasAll = selectedCategories.value.includes(allCategoryFilter)
    selectedCategories.value = wasAll && next.length > 1 ? next.filter((item) => item !== allCategoryFilter) : [allCategoryFilter]
    return
  }
...
const tableRows = computed(() => {
  ...
  }).filter((row) => !categoryFilterEnabled.value || selectedCategories.value.includes(row.category))
})
  ...
  <section class="keyword-dashboard" aria-label="Amazon-ABA排名">
    ...
    <section class="keyword-table-panel" aria-label="搜索词周度数据">
      <div class="keyword-table-wrap">
        ...
        <div v-else-if="!tableRows.length && !loading" class="keyword-empty">{{ categoryFilterEnabled ? "当前分类没有关键词" : "点击左下角 + 添加关键词，选择分类后保存。" }}</div>
      </div>
