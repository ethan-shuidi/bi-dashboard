...
const AmazonStrategyBoard = defineAsyncComponent(() => import("./AmazonStrategyBoard.vue"))
...
function showProductTooltip(row, event) {
  const rect = event.currentTarget.getBoundingClientRect()
  const width = 180
  const height = 32
  const left = Math.max(8, Math.min(window.innerWidth - width - 8, rect.left))
  const top = rect.top - height - 7 >= 8 ? rect.top - height - 7 : rect.bottom + 7
  productTooltip.value = { asin: row.metrics?.asin || "", top, left }
}

function hideProductTooltip() {
  productTooltip.value = null
}
...
onMounted(async () => {
  window.addEventListener("scroll", updateColumnConfigPosition, true); window.addEventListener("resize", updateColumnConfigPosition)
...
onBeforeUnmount(() => {
  window.removeEventListener("scroll", updateColumnConfigPosition, true); window.removeEventListener("resize", updateColumnConfigPosition)
  resizeCleanup?.()
...
<template>
  <div class="amazon-page">
    ...
    <section class="amazon-data-module" aria-label="产品经营数据模块">
